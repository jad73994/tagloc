...
%
function ret = read_bf_file_channel(filename)
%% Input check
...
while cur < (len - 3)
    ...
    if (code == 187) % get beamforming or phy data
        ...
        if (length(bytes) ~= field_len-1)
            ret = ret(1:count);
            fprintf('Broken count : %d, Count : %d',broken_count,count);
            fclose(f);
            return;
        end
    ...
    if (code == 187) %hex2dec('bb')) Beamforming matrix -- output a record
        count = count + 1;
        ret{count} = read_bfee(bytes([1:20,89:end]));
        ret{count}.mac=bytes(32);
        ret{count}.next_channel=bytes(33);
        ret{count}.ht40=bytes(34);
        ret{count}.packet_id=typecast(bytes(39:1:42),'uint32');
        ret{count}.tv_sec = typecast(bytes(81:1:84),'uint32');
        ret{count}.tv_usec = typecast(bytes(85:1:88),'uint32');
        perm = ret{count}.perm;
        Nrx = ret{count}.Nrx;
...
ret = ret(1:count);
fprintf('Broken count : %d, Count : %d\n',broken_count,count);
%% Close file
fclose(f);
end
