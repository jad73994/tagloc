...
%
function ret = read_bf_file_timestamp_mac(filename)
coder.extrinsic('fopen','fseek','ferror','fclose','ftell','cell','fread')
...
while cur < (len - 3)
    ...
    if (code == 187) %hex2dec('bb')) Beamforming matrix -- output a record
        count = count + 1;
        ret{count} = read_bfee_timestamp_mac(bytes);        
        perm = ret{count}.perm;
        Nrx = ret{count}.Nrx;
        if Nrx == 1 % No permuting needed for only 1 antenna
            continue;
        end
        %Swarun: modified for 2 antenna receivers
