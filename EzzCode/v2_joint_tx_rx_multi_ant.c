...
int main(int argc, char *argv[])
{
  ...
  if (argc != 5) {
    fprintf(stderr, "Usage: %s carrier_freq number_of_seconds number_of_antennas joint_tx_file\n", argv[0]);
    exit(1);
  }
        int fd_lte_frame;
	int read_ret;
	int buf_size=640*140*8;
	int write_ret;
	...
	int num_logged_subframes=0;
        int max_peak_value=0;
        int8_t ch_slope_samples=0;
...
//////////////////// loading tx file in memory

	fd_lte_frame = open(argv[4], O_RDONLY, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
	...
	read_ret=read(fd_lte_frame, (char *)(read_buf),buf_size);
	if(read_ret!=buf_size){
		printf("error read function returned %d \n",read_ret);
		return -1;
	}
	close(fd_lte_frame);
...
///////////////////////////////////////////////
	...
	bbregs[54]=controller_scale;// -(controller_scale*tx_carrier_freq/rx_carrier_freq);    //  TX CFO scaling
	bbregs[55]=(controller_scale*15360*30.72)/(7*32*carrier_freq); // TX SFO scaling (negative of the RX SFO scaling)
		...
		} else
		{
			...
			if((fine_cfo_state==3)&&(((rx_mem_pointer&0x1FFF)-fine_cfo_sf)>30))
			{
				...
				channel_slope=atan2(fine_cfo_imag,fine_cfo_real)/6.0;
                                ch_slope_samples=(0.5+(channel_slope/0.003068));
			...
			if((fine_cfo_state==4)&&(((rx_mem_pointer&0x1FFF)-fine_cfo_sf)>50))
			{
/////////////////////////////////////////////////////////////////////////// Tx joint 
				bbregs[33]=bbregs[32] + (buf_size/getpagesize()) -1;

				bbregs[27]=(5357<<14)+2793; 
				bbregs[28]=640; 
				bbregs[17]=3;
                                while(((bbregs[11]&0xFF)<max_peak_value)||(bbregs[12]<2)){usleep(100);};
				bbregs[18]=(((bbregs[11]>>10)-ch_slope_samples+293998-27)-6474+307200)%307200;
				bbregs[19]=0;
				bbregs[20]=0;
				bbregs[21]=0;
				bbregs[22]=25758 * 3;
				bbregs[23]=0x0F;
				bbregs[4]&=(~((0x1<<16)+(0x1)+(0x1<<2)));
				bbregs[19]=-bbregs[5];
				bbregs[20]=-bbregs[15];
                                rx_mem_pointer=bbregs[26];
                                fine_cfo_sf=rx_mem_pointer&0x1FFF;
                                ++fine_cfo_state;
				if(1){
					printf("current subframe= %d , starting joint tx!\n",fine_cfo_sf);
				}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
